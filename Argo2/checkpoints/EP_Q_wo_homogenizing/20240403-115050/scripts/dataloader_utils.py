...
class DataLoaderAV2(object):
    ...
    def __init__(self, 
        ...
        self.split = split
        self.file_dataset = generate_file_list_dataset("data/datasets/" + split + "_processed"+ "_" + dataset)
    ...
    def _load_data(self, path_track, path_map, flag_flip):
        ...
        output:
            dict containing: 
                flat features of agents, dim [num_max_agents, LEN_AGENT_INPUT]
                flat features of map elements, dim [num_max_mapels, LEN_MAPEL_INPUT]
                mask for neighboring agents, dim [num_max_agents, num_max_agents] # TODO: whether ues np.triu_indices to avoid redundant values
                mask for neighboring map elements of agents, dim [num_max_agents, num_max_mapels]
                mask for neighboring map elements, dim [num_max_mapels, num_max_mapels] # TODO: whether ues np.triu_indices to avoid redundant values
        '''
        # load data:
        ...
        mask_agents = np.ones(num_agents, dtype=bool)
        mask_mapels = np.ones(num_mapels, dtype=bool)
        ...
        x = np.zeros([num_agents, 50, 2], dtype=np.float32)
        ...
        if track_info['y'] is not None:
            ...
            #y[:num_agents] = (track_info['y'][:, :, :2] - agents_origin[:, None, :]) # x,y positions
        ...
        x_clone = deepcopy(x)
        x[:num_agents, 1:50, :] = np.where(timestep_x_mask, track_info['x'][:num_agents, 1:, :2] - track_info['x'][:num_agents, :-1, :2], x_clone[:num_agents, 1:50, :])
        ...
        return (x,
            ...
            y_mask_all,   # [A, 60]
            y_mask_others,   # [A, 60]
            track_info['timestep_x_mask'][:, -1],   # [A]
            mask_mapels,    # [M]
    ...
    def _create_dict(self, 
        ...
        return {"x": x,
                ...
                "mask_agents": mask_agents,
                "mask_mapels": mask_mapels,
    ...
    def load_process(self, shuffle = False, flip = False):
        ...
        self.loaded_dataset = (self.combined_dataset.map(lambda path_data, flag_flip: tf.numpy_function(self._load_data, (path_data[0], path_data[1], flag_flip), 
                                    [tf.float32, tf.float32, tf.float32, tf.float32, bool, bool, bool, bool, bool, bool, bool, tf.int32, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, bool, bool, bool, tf.string, tf.string]), num_parallel_calls=tf.data.AUTOTUNE))

        self.loaded_dataset = self.loaded_dataset.cache()
        
        # Pad after cache to use less memory
        self.loaded_dataset = self.loaded_dataset.padded_batch(self.batch_size, padded_shapes = ([self.num_max_agents, None, None], # x
                                                                                                 [self.num_max_agents, None, None], # y
                                                                                                 ...
                                                                                                 [self.num_max_agents], # mask_agents
                                                                                                 [self.num_max_mapels], # mask_mapels
                                                                                                 [None], # target_agents
                                                                                                 [None], # origin
                                                                                                 [None, None], # ro_mat
