...
class EP_F(EP_Base):
    ...
    def call(self, data, training):
        ...
        input_a_rel = tf.concat([agents_origin,  s_agent_theta, c_agent_theta], axis=2)
...
class EP_Q(EP_Base):
    ...
    def call(self, data, training):
        ...
        input_a_cps = (input_a_cps - tf.squeeze(agent_t, axis=2)) @ tf.squeeze(agent_R, axis=2) # [B, A, 6, 2]
        ...
        c_mapel_theta, s_mapel_theta = tf.cos(mapels_theta[:, :, None]), tf.sin(mapels_theta[:, :, None])
        mapel_R = tf.reshape(tf.concat([c_mapel_theta, -s_mapel_theta, s_mapel_theta, c_mapel_theta], axis=-1), [B, M, 2,2])  # [B, M, 2, 2]
        #input_m_cps = tf.repeat(input_m_cps[:, None, :, :, :], A, axis= 1) #[B,A,M,4,2]
        #input_m_cps = (input_m_cps - agent_t) @ agent_R # [B, A, M, 4, 2]
        input_m_cps = (input_m_cps - mapels_origin[:, :, None, :]) @ mapel_R  # [B, M, 12(4), 2]
        ...
        input_m_cps = tf.reshape(input_m_cps, (B, M, -1, 4, 2))
        input_m_cps_vec = input_m_cps[:, :, :, 1:, :] - input_m_cps[:, :, :, :-1, :] #[B,M, 3(1), 3,2]
        input_m_cps_vec = tf.reshape(input_m_cps_vec, [B,M,-1])
        ...
        mapels_emb = self.mapel_embed_layer(input_m_cps_vec, m_other_features)   # [B, M, d_hidden]    
        
        agents_emb = tf.reshape(agents_emb[:, None, :, :] + a_rel_embed, [B*A, A, -1])
        mapels_emb = tf.reshape(mapels_emb[:, None, :, :] + m_rel_embed, [B*A, M, -1])
        ...
        mapels_emb = self.map_encoder(q=mapels_emb, masks=mask_neighbor_mapel, training=training) #+ mapels_time # [B*A, M, d_hidden]
        ...
        for i in range(self.n_stacked_encmodule):
            ...
            agents_emb = self.agent_map_encoder[i](q=agents_emb, k=mapels_emb, masks=mask_neighbor_agent_mapel, training=training) # [B*A, A, d_hidden]
