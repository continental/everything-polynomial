...
from utils_common import loss_utils
import utils_common.helper_utils as utils
import utils_common.loss_utils as loss_utils
...
class EP(tf.keras.Model):
    ...
    def train_dataset(self, 
        ...
        shutil.copy("utils_common/dataloader_utils.py", log_dir + "/scripts/")
        shutil.copy("utils_common/loss_utils.py", log_dir + "/scripts/")
        ######################################################################
    ...
    def eval_dataset(self, val_dataset, eval_length=60, target = "focal"):
        batch_losses = []

        for data in tqdm(val_dataset, desc="val it", leave = False):
            loss = self.eval_step(data, eval_length = eval_length, target = target)
            batch_losses.append(loss)
    ...
    @tf.function
    def eval_step(self, data, eval_length, target):
        prediction = self(data, training = False)
        y_target, prediction_target = self.get_target(data, prediction, target)

        loss = [m.call(y_target, prediction_target['traj_mean'], pi = prediction_target['logits'], length = eval_length) for m in self.val_metrics]
    ...
    def call(self, data, training, origin = None, ro_mat = None) -> tf.Tensor:
        ...
        if self.model_name == 'EP_F':
            # EP_F will predict other agents in the "focal agent" frame (shifted)
            ...
            pred_traj_mean_others = pred_traj_mean_others @ prediction['agent_R'][:,:, None, :, :]
